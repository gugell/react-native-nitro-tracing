...
export function createPerformancePlugin(api: PerformancePluginOptions): TracePlugin {
  return { id: 'performance', start({ recording, reportError }) {
    ...
    const observer = new api.PerformanceObserver((list) => {
      ...
      for (const entry of list.getEntries()) {
        try {
          ...
          const data = entry as typeof entry & { value?: unknown; detail?: { id?: unknown; unit?: unknown } }
          const context = {
            name: entry.name.slice(0, 128),
            correlationId: typeof data.detail?.id === 'string' ? data.detail.id.slice(0, 128) : '',
            attributes: [{ key: 'source', value: 'performance' }, { key: 'source.startTimeMs', value: String(entry.startTime) }],
          }
          if (entry.entryType === 'measure') recording.recordSpan({ ...context, timestampMs, durationMs: entry.duration, outcome: 'success' })
          else if (entry.entryType === 'mark') recording.mark({ ...context, timestampMs })
          else if (entry.entryType === 'metric' && typeof data.value === 'number' && Number.isFinite(data.value))
            recording.recordMetric({ ...context, timestampMs, value: data.value, unit: typeof data.detail?.unit === 'string' ? data.detail.unit.slice(0, 32) : 'unspecified' })
        } catch (error) { reportError(error) }
      }
    })
    try { observer.observe({ entryTypes: ['mark', 'measure', 'metric'] }) }
    catch (error) { observer.disconnect(); throw error }
    return { stop() { stopped = true; observer.disconnect() } }
  } }
}
